...
- the performance baseline job builds `lean-assumptions` and
  `LeanAssumptionsTest.Fixtures` before running CLI smoke checks
...
- the release-readiness job builds `lean-assumptions` and
  `LeanAssumptionsTest.Fixtures` before running CLI smoke checks
