...
| FR-012 conservative failure mode | partial | Core expansion emits `unknown` for cycle/depth/metadata failures and strict policy fails unknowns. However, unrecognized binder-type head shapes (`let` wrappers, non-structure inductives carrying proofs, function types into proof-carrying data, `def`-based aliases, and alias-headed declaration types) currently default to `pure_data` and can pass strict policy silently. Confirmed by audit on 2026-07-11. |
