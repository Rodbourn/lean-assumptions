...
| FR-006 alias transparency policy | partial | `none` reports `abbrev` aliases as `alias` nodes, but aliases introduced with `def` or `@[reducible] def` are not detected as aliases and conservatively classify as `unknown` rather than `alias`. `reducible` currently reduces at default transparency, unfolding more than documented, and `recursive_normalization` is not operationally distinct from it. No output signal exists when classification depends materially on transparency. Confirmed by audit on 2026-07-11. |
| FR-007 cycle detection | partial | Recursive expansion is fuel-bounded and cycle-safe, but cycle detection keys on structure head names rather than type instances, so nested same-head generics such as `Nat × Nat × Nat` are misreported as truncated cycles and fail strict policy. Confirmed by audit on 2026-07-11. |
...
| FR-012 conservative failure mode | partial | Classification is by positive head recognition: unrecognized binder-type head shapes (including `def`-based aliases and opaque heads) emit `unknown` and fail strict policy, `let` wrappers reduce structurally, non-structure inductives are scanned constructor by constructor, and function types classify through their result type. Remaining gap: a declaration type hidden entirely behind an alias is still peeled to zero binders without auditing the result surface. |
